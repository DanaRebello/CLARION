...
def candidate_dashboard():
    ...
    with get_db() as conn:
        ...
        WHERE a.candidate_id=?""",(session['user'],)).fetchall()

        # Notifications
        notifications = conn.execute("""
        SELECT message, timestamp FROM notifications
        WHERE candidate_id=? ORDER BY timestamp DESC
    ...
    notif_html = ""
    if notifications:
        notif_html += "<h3>Notifications</h3><ul>"
        for n in notifications:
            notif_html += f"<li>{n[0]} <small>({n[1]})</small></li>"
        notif_html += "</ul>"
...
def update_candidate():
    ...
    with get_db() as conn:
        ...
        WHERE id=?""",(request.form['qualification'],
             request.form['skills'],
...
def apply(job_id):
    with get_db() as conn:
        ...
        VALUES(?,?,?)""",(job_id, session['user'], score))
...
def recruiter_dashboard():
    ...
    for a in apps:
        # Row for pending applications (with Shortlist/Reject buttons)
        row = f"""
        <tr>
        <td>{a[2]}</td>
        <td>{a[3]}</td>
        ...
        # Row for shortlisted candidates (no Action column)
        if a[7] == "shortlisted":
            shortlisted += f"""
            <tr>
            <td>{a[2]}</td><td>{a[3]}</td><td>{a[4]}</td>
            <td><a href="/resume/{a[1]}">Download</a></td>
            <td>{round(a[6],2)}</td><td>{a[7]}</td>
            </tr>
            """
        elif a[7] == "pending":
            pending += row
        else:  # rejected, do not show
            continue
    ...
    <tr>
    <th>Name</th><th>Skills</th><th>Experience</th>
    <th>Resume</th><th>Score</th><th>Status</th>
    </tr>
    {shortlisted}
    </table>
    """
...
def update(app_id, status):
    with get_db() as conn:
        if status == "shortlisted":
            ...
            # Get candidate id, job title, company
            result = conn.execute("""
            SELECT candidate_id, j.title, j.company 
            FROM applications a
            JOIN jobs j ON a.job_id=j.id
            WHERE a.id=?
            """, (app_id,)).fetchone()

            candidate_id, job_title, company = result
            message = f"Congratulations! You are shortlisted for the post of {job_title} at {company}."
            
            # Insert notification
            conn.execute("INSERT INTO notifications(candidate_id, message) VALUES(?,?)", 
...
>>>>>>> 630fdc24dde4654c1494a6d0ca20df30678f2a7e
