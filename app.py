...
# ---------------- LOGIN LOGGER ----------------
def log_login(user_id, user_type):
...
def candidate_dashboard():
    ...
    apps_html = "".join(f"<li>{a[0]} | {a[1]} | {a[2]}</li>" for a in apps)

    notif_html = "<h3>Notifications</h3><ul>" + "".join(
        f"<li>{n[0]} ({n[1]})</li>" for n in notifications
...
def recruiter_dashboard():
    ...
    with get_db() as conn:
        apps = conn.execute("""
        SELECT a.id,c.id,c.name,c.skills,c.experience,
    ...
    <h3>Applications</h3>
    <table border="1">
    <tr><th>Name</th><th>Skills</th><th>Experience</th>
    <th>Score</th><th>Status</th><th>Action</th></tr>
    {rows}
    </table>
    """
